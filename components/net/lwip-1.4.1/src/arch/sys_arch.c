...
#include <string.h>
// added by luoyang @2017-2-16
#if HT_USING_EEPROM_STORE_IP_INFO
#include "matrix_common.h"
#include "ht_proto_process.h"
#endif
...
int lwip_system_init(void)
{
    ...
    if (netif_default != RT_NULL)
    {
        struct ip_addr ipaddr, netmask, gw;
			...
			Flag = matrix_eeprom_read_byte(EEPROM_NetWork+EEPROM_NetWork_Flag);
			if(Flag == 'Y')
			{
				matrix_eeprom_read_bytes(EEPROM_NetWork+EEPROM_NetWork_Data,Data,12);
			}
			else
			{
				...
				u8 i= 0;
				//ip
				Data[i++] =  RT_LWIP_IPADDR0;
				Data[i++] =  RT_LWIP_IPADDR1;
				Data[i++] =  RT_LWIP_IPADDR2;
				Data[i++] =  RT_LWIP_IPADDR3;
				//netmask
				Data[i++] =  RT_LWIP_MSKADDR0;
				Data[i++] =  RT_LWIP_MSKADDR1;
				Data[i++] =  RT_LWIP_MSKADDR2;
				Data[i++] =  RT_LWIP_MSKADDR3;
				//gateway
				Data[i++] =  RT_LWIP_GWADDR0;
				Data[i++] =  RT_LWIP_GWADDR1;
				Data[i++] =  RT_LWIP_GWADDR2;
				Data[i++] =  RT_LWIP_GWADDR3;

			}
			IP4_ADDR(&ipaddr, Data[0], Data[1], Data[2], Data[3]);
      IP4_ADDR(&netmask, Data[4], Data[5], Data[6], Data[7]);
			IP4_ADDR(&gw, Data[8], Data[9], Data[10], Data[11]);
#else
        IP4_ADDR(&ipaddr, RT_LWIP_IPADDR0, RT_LWIP_IPADDR1, RT_LWIP_IPADDR2, RT_LWIP_IPADDR3);
        IP4_ADDR(&gw, RT_LWIP_GWADDR0, RT_LWIP_GWADDR1, RT_LWIP_GWADDR2, RT_LWIP_GWADDR3);
        IP4_ADDR(&netmask, RT_LWIP_MSKADDR0, RT_LWIP_MSKADDR1, RT_LWIP_MSKADDR2, RT_LWIP_MSKADDR3);
#endif // end HT_USING_EEPROM_STORE_IP_INFO
